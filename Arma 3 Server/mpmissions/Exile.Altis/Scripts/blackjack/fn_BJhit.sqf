...
switch ( _NextCard ) do{	
case 3: { _Pcard3 ctrlSetText format["%1",_number]; 
 _P3T = ctrlText _Pcard3; 
 }; 
	case 4: { _Pcard4 ctrlSetText format["%1",_number]; 
 _P4T = ctrlText _Pcard4; 
 }; 
	case 5: { _Pcard5 ctrlSetText format["%1",_number]; 
 _P5T = ctrlText _Pcard5; 
 }; 
	default { _Pcard3 ctrlSetText format["%1",_number]; 
 _P3T = ctrlText _Pcard3; 
 }}; 
_totalAces = 0; 
//check if there are any aces
//and where they are
if( _P1T == "ACE" ) then{	_ACES = true; 
	_ACE1 = 1; 
	_totalAces = 1; 
}; 
if( _P2T == "ACE" ) then{	_ACES = true; 
	if( _ACE1 == 0 ) then	{		_ACE1 = 2; 
		_totalAces = 1; 
	}	else	{		_ACE2 = 2; 
		_totalAces = 2; 
	}; 
	}; 
if( _P3T == "ACE" ) then{	_ACES = true; 
	if( _ACE1 == 0 ) then	{		_ACE1 = 3; 
		_totalAces = 1; 
	}	else	{		if( _ACE2 == 0 ) then		{			_ACE2 = 3; 
			_totalAces = 2; 
		}		else		{			_ACE3 = 3; 
			_totalAces = 3; 
		}; 
...
if( _P4T == "ACE" ) then{	_ACES = true; 
	if( _ACE1 == 0 ) then	{		_ACE1 = 4; 
		_totalAces = 1; 
	}	else	{		if( _ACE2 == 0 ) then		{			_ACE2 = 4; 
			_totalAces = 2; 
		}		else		{			if( _ACE3 == 0 ) then			{				_ACE3 = 4; 
				_totalAces = 3; 
			}			else			{				_ACE4 = 4; 
				_totalAces = 4; 
			}; 
...
if( _P5T == "ACE" ) then {		_ACES = true; 
	if( _ACE1 == 0 ) then	{		_ACE1 = 5; 
		_totalAces = 1; 
	}	else	{		if( _ACE2 == 0 ) then		{			_ACE2 = 5; 
			_totalAces = 2; 
		}		else		{			if( _ACE3 == 0 ) then			{				_ACE3 = 5; 
				_totalAces = 3; 
			}			else			{						_ACE4 = 5; 
				_totalAces = 4; 
			}; 
...
if( _ACES ) then {	if( _ACE4 != 5 OR _ACE3 != 5 OR _ACE2 != 5 OR _ACE1 != 5 ) then 	{		
...
if( _win != 0 ) then 	{		
_cash = _betamt * _win; 
		_newPoptabs = ExileClientPlayerMoney + _cash;
